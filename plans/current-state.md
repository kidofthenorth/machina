...
- **DOING:** M4 (sweep / walk-forward). Plan = [m4-sweep.md](m4-sweep.md) (12 subtasks S1–S12).
  **S1–S9 DONE** — the *deterministic sweep core* (S1–S7), the **walk-forward window model (S8)**, and
  the **holdout gate (S9)**: existing-crate prep (S1–S3) + `crates/sweep` skeleton (S4), param grid +
  ...
  **physically moves** the holdout into a separate `Vec<Bar>` via `split_off`; rejects overlap/empty/
  ...
  doc). **Next diff: S10** — cost/fee sensitivity ladder. Remaining: S10 cost sensitivity,
  S11 report+schema, S12 CLI.
...
- Continue M4 per [m4-sweep.md](m4-sweep.md): S1–S9 are DONE (deterministic sweep core + walk-forward
  windows + sealed holdout). Next is **S10** — the cost/fee sensitivity ladder (`sweep/src/
  sensitivity.rs` + `baseline.rs`: `before_costs=zero`, `base`, `doubled` each cost field ×2 exact;
  baselines re-run cost-matched) — then S11 (advancement/rejection report + new
  `schemas/sweep-report.schema.json`), S12 (CLI `sweep`/`sweep-verify` + plans/docs/DECISIONS refresh).
  Each subtask is a small diff that keeps the workspace green. Do NOT start execution (M8/M9) without
