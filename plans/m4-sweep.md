...
| S4 | TODO | `crates/sweep` skeleton + workspace wiring | `Cargo.toml`, `sweep/Cargo.toml`, `sweep/src/lib.rs` | — | `cargo build`/clippy/fmt green; `no-execution-deps` passes |
| S5 | TODO | Param model + grid + strategy bridge | `sweep/src/param.rs`, `lib.rs` | S4 | Cartesian product fixed-order across two builds; `build_strategy` round-trips; `param_id` scale-canonical |
| S6 | TODO | Single-cell runner + exact Decimal turnover + NaN normalization | `sweep/src/cell.rs`, `sweep/src/turnover.rs` | S1,S3,S5 | `eval_cell` matches direct `run`+`Metrics`; turnover exact vs `dec!()`; fwd-vs-rev sum identical; no raw NaN |
| S7 | TODO | **Determinism gate** — `std::thread::scope` scheduler | `sweep/src/parallel.rs`, `sweep/tests/determinism.rs` | S6 | `run_cells(cells,K)==sequential` structural + byte-identical for K∈{1,2,3,7,8}; repeated-run byte-identity; explicit thread counts |
| S8 | TODO | Walk-forward window model | `sweep/src/window.rs`, `sweep/tests/walk_forward.rs` | S4 | `train.end<=test.start` w/ embargo; tests ordered+non-overlapping; no OOB; rolling/anchored shapes; bad params rejected |
| S9 | TODO | **Holdout gate** — partition + physical seal + consume-by-value | `sweep/src/partition.rs`, `sweep/src/config.rs`, `sweep/tests/holdout_sealing.rs` | S2,S6,S8 | read-counter==0 + digest unchanged after full pipeline; `compile_fail` doctest; `evaluate_on_holdout` call-once; overlap rejected |
| S10 | TODO | Cost/fee sensitivity ladder + cost-matched baselines | `sweep/src/sensitivity.rs`, `sweep/src/baseline.rs` | S6 | doubled doubles each field exactly; `before_costs==zero()`; doubled shows ≥ base fees, ≤ base net return |
