...
**GATE DECLARED 2026-07-09** — evidence in worklog + task-queue M4 cards.
...
> **S11 note.** As shipped, `SweepReport` carries only the rejection report (thresholds, `trial_count`,
> per-candidate verdicts) — no f64 stats, so `results::stat_to_string` is not needed yet. The
> status⇔`failed_criteria` coupling (`advanceable` iff the list is empty) is enforced in **code**
> (`evaluate_candidate`), not the schema, mirroring the repo's "schema checks shape, code enforces
> semantics" convention (cf. run-result `unitInterval`). Verdicts sort by a **total** order (label,
> then status/reasons) so output is byte-identical even if two candidates share a label.

## 13. CLI wiring

> **Reality check (2026-07-06 audit).** As of S11 the sweep crate exposes only *primitives* — there is
> no `SweepSpec`, no `run_sweep`, and no TOML `Deserialize` anywhere in `crates/sweep` (the `toml` dep
> is declared but unused), and `strategy-lab.example.toml` has no `[walk_forward]`/`[advancement]`
> blocks or grid arrays. The exact template paths are `config/strategies/strategy-lab.example.toml`
> and `config/tokens/allowlist.example.toml`. This section's design therefore lands via **task cards
> M4-C2…M4-C7 in [task-queue.md](task-queue.md)** (spec parsing C2 → runner C3–C5 → CLI C6–C7), which
> carry the exact signatures and steps.
...
- `machina sweep [--threads N] [--out PATH]` — loads the embedded
  `config/strategies/strategy-lab.example.toml` + `config/tokens/allowlist.example.toml`, builds a
  `SweepSpec` (families from the `[trend_alloc_v1]` / `[threshold_rebalance_v1]` grids — **note config
  key `rebalance_band` maps to struct field `band`** — cost scenarios before/base/doubled, walk-forward
  over the development partition), runs `run_sweep` with `Parallelism::Threads(N)` (default
  `Sequential`), prints/writes canonical JSON.
...
| S11 | ✅ DONE | Advancement/rejection + `SweepReport` export + new schema | `sweep/src/advance.rs`, `report.rs`, `schemas/sweep-report.schema.json`, `sweep/tests/schema_validation.rs` | S7,S9,S10 | DONE: `advance.rs` (Decimal-only `AdvancementThresholds`/`CandidateEvidence`/`Verdict`/`RejectionKind`×7/`RejectionReason`{observed,threshold decimal-strings}/`evaluate_candidate` — InsufficientData hard-stop, fixed canonical order, edge-vanishes derived from `doubled_return<=floor` single-source); `report.rs` `SweepReport` (robustness-only `note`, `trial_count`, total-order sort so byte-identical even with duplicate labels); new `sweep-report.schema.json` (Draft 2020-12, additionalProperties:false, decimalString; D-0001). Gate green: real report validates; each criterion→matching reason; `trial_count` recorded; f64-audit (source-level); two serializations byte-identical; rejects numeric budget / unknown enums / extra props; run-result schema UNCHANGED. Adversarially reviewed (3-lens + skeptics): schema fidelity clean; 3 nit fixes (single-source edge-vanishes, total-order sort, coupling-is-code-enforced note); 1 refuted (f64-audit scope is intentional) |
| S12 | ✅ DONE (as cards M4-C1…C10) | CLI `sweep`/`sweep-verify` + refresh plans/docs/DECISIONS — **expanded 2026-07-06 into task cards M4-C1…M4-C10 in [task-queue.md](task-queue.md)** (the audit found S12 underspecified: spec parsing, config-template blocks, and the runner/evidence aggregation did not exist and are prerequisites for the CLI) | `config/strategies/strategy-lab.example.toml`, `sweep/src/{spec,runner}.rs` (new), `sweep/src/{lib,partition}.rs`, `cli/src/main.rs`, `cli/Cargo.toml`, `plans/*`, `docs/architecture-index.md`, `AGENTS.md`, `DECISIONS.md` (D-0009) | S11 | per-card gates in task-queue.md; ends with the M4 gate declaration card (parallel==sequential byte-identical; repeated runs identical; holdout untouched; full workspace green) — **GATE DECLARED 2026-07-09** |
